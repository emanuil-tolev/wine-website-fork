...
class wwn 
{
    ...
    public function get_list ()
    {
        ...
        while($file = readdir($dir))
        {
            ...
            {
                case "interviews":
                    $issue = preg_replace("/interview_([0-9]+)\.xml/", "\\1", $file);
                    break;

                default:
                    $issue = preg_replace("/wn[0-9]+_([0-9]+)\.xml/", "\\1", $file);
            }
            if ($issue > $old)
                $cur = $issue;
            $old = $issue;
    ...
    // show back issues
    public function issues_list ($issues, $cur, $limit = 0, $pos = 0)
    {
    ...
    private function format_summary ($cur, $date = null)
    {
        global $html;
    ...
    public function view_issue ($view = "latest")
    {
    ...
    private function wwn_xml_parse ($file)
    {
        global $config, $html;
...
function _translateWWN2valid ($xmlSource)
{
    ...
    $xmlSource = preg_replace("/\&/", "&#038;", $xmlSource);
    // strip out comments
    $xmlSource = preg_replace("/<\!-- .* -- \/>/", "", $xmlSource);
    return $xmlSource;
}
